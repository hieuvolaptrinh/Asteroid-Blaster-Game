...
/* drawShipBody(): Ve than phi thuyen voi hieu ung chieu sau va mau sac dep. */
void drawShipBody(Ship s, float angle)
{
  ...
  /* Lop bong (shadow) */
  {
    ...
    int slx = lx + 2, sly = ly + 2;
    int srx = rx + 2, sry = ry + 2;
  ...
  /* Highlight LIGHTCYAN nho nhat */
  {
    float hs2 = 0.4f;
    int h2nx = (int)(s.x + cosf(angle) * (s.radius + 14) * 0.8f);
    int h2ny = (int)(s.y + sinf(angle) * (s.radius + 14) * 0.8f);
  ...
  /* Kinh lai */
  {
    ...
    fillellipse(cx - 1, cy - 1, 2, 2);
    setcolor(LIGHTCYAN);
    ellipse(cx, cy, 0, 360, 6, 5);
  }
}

/* drawShipEngineFlame(): Ve luoi lua dong co. */
void drawShipEngineFlame(Ship s, float angle, int frame)
{
  ...
  for (eng = 0; eng < 2; eng++)
  {
    ...
    setfillstyle(SOLID_FILL, ((frame + eng) % 3 == 0) ? LIGHTRED : RED);
...
/* drawShipShieldAura(): Ve hao quang bao ve quanh tau. */
static void drawShipShieldAura(Ship s, int frame)
{
  int auraR = s.radius + 8 + (frame % 8) / 2;
  setcolor(BLUE);
  circle((int)s.x, (int)s.y, auraR + 3);
  setcolor((frame / 4) % 2 ? LIGHTCYAN : CYAN);
